...
/*!
    ...
    \sa getLowerCaseCharFromUser(), getIntFromUser(), getStringFromUser()
    ...
    \sa getStringFromUser(), getBoolFromUser()
    ...
    \sa getStringFromUser(), getUnsignedIntFromUser()
    ...
    \sa getStringFromUser(), getPositiveDoubleFromUser(), getXYZDoubleVec3DFromUser()
    ...
    \sa getDoubleFromUser(), getRGBDoubleVec3DFromUser()

    \fn bool getBoolFromUser(std::string question = "", std::string prompt = PROMPT)
    \brief Gets a boolean from the user.
    \details Uses the getLowerCaseCharFromUser() method. Will continue to ask the question until the user gives 't' or 'f'.
    \param question The question that will be asked to the user.
    \param prompt The prompt that will be used to show the user that he or she can write something.
    ...
    \details Uses three times the getPositiveDoubleFromUser() method. Adds "r ", "g " or "b " before the prompt, to show the user which component he or she is giving.
    \param question The question that will be asked to the user.
    \param prompt The prompt that will be used to show the user that he or she can write something.
    \return The vector the user gave.
    ...
    \sa createRefractiveMaterial(), createSpecularMaterial(), createMaterial()
    ...
    \sa createDiffuseMaterial(), createSpecularMaterial(), createMaterial()
    ...
    \sa createDiffuseMaterial(), createRefractiveMaterial(), createMaterial()

    \fn Material* createMaterial()
    \brief Interactive creation of a Material.
    \details Gets the main parameters of a material, and then calls createDiffuseMaterial(), createRefractiveMaterial() or createSpecularMaterial().
    ...
    \sa createTriangle(), createObject3D()
    ...
    \sa createSphere(), createObject3D()

    \fn Object3D* createObject3D()
    \brief Interactive creation of an Object3D.
    \details Gets the main parameters of an object, and then calls createSphere() or createTriangle().
    ...
    \sa to_json(json& j, const Object3D& obj)
*/
